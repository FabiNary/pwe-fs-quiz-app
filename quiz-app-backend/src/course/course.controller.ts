...
export class CourseController {
  ...
  async getStudentSolutionsAsZip(
    ...
    res.header(
      'Content-Disposition',
      `attachment; filename="${generateFilename(courseName)}-solutions.zip"`,
    );

    const zipFile =
      await this.courseService.getStudentSolutionsAsZip(courseName);
