...
export class StudentController {
    ...
    @ApiOperation({ summary: 'Fügt Studenten aus einer CSV-Datei zu einem Kurs hinzu' })
    @ApiConsumes('multipart/form-data')
    @ApiBody({ description: 'CSV-Datei mit Studenten', type: AddStudentsAsCsvDto })
    ...
    async addStudentsAsCsv(
        ...
        this.studentService.notifyStudentsToCreateQuiz(courseName, students).then(r => r);
        return { message: `Studenten erfolgreich zum Kurs '${courseName}' hinzugefügt` };
    }

    @Get('getStudentsByCourse/:course')
    @ApiOperation({ summary: 'Gibt alle Studierenden eines bestimmten Kurses zurück' })
    @ApiResponse({
        status: 200,
