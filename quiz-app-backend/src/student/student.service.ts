...
export class StudentService {
    ...
    addStudentsFromCsv(courseName: string, csvContent: string): void {
        ...
        writeFileSync(join(courseDir, 'students.json'), JSON.stringify(students, null, 2));
    }
    ...
    getStudentsByCourseDir(dirName: string): StudentDto[] {
        ...
        const students: StudentDto[] = JSON.parse(readFileSync(studentsFile, 'utf-8'));
        return students;
    }

    async notifyStudentsToCreateQuiz(courseName: string, students: StudentDto[]): Promise<void> {
    ...
    notifyStudentsQuizAvailable(courseName: string, students: StudentDto[], quizId: string, duration: Date): void {
        const baseUrl = 'http://localhost:3000';
