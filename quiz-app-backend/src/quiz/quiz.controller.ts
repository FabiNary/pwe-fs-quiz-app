...
export class QuizController {
  ...
  addQuiz(
    ...
    return {
      message: `Quiz '${quizId}' erfolgreich zum Kurs '${courseDir}' hinzugefügt`,
    };
  }
  ...
  addQuizSolution(
    ...
    if (!editableTill || editableTill < dateNow) {
      throw new BadRequestException(
        `Das Quiz '${quizId}' darf nicht mehr bearbeitet werden. Bearbeitungsfrist war '${editableTill}'`,
      );
    }

    this.quizService.addSolutionToQuiz(courseDir, quizId, studentId, solution);
    return {
      message: `Lösung für Quiz '${quizId}' von Student '${studentId}' erfolgreich hinzugefügt`,
    };
  }

  @Patch('updateEditableTill/course/:courseName/quiz/:quizId/:timestamp')
  @ApiOperation({
    summary: 'Aktualisiert den Bearbeitungszeitraum eines Quizzes',
  })
  updateEditableTill(
    @Param('courseName') courseName: string,
    @Param('quizId') quizId: string,
    ...
    this.studentService.notifyStudentsQuizAvailable(
      courseName,
      students,
      quizId,
      newEditableTill,
