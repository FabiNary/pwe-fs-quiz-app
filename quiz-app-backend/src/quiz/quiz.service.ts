...
export class QuizService {
  ...
  getQuestionsWithoutAnswers(courseDir: string, quizId: string) {
    ...
    if (!existsSync(quizPath)) {
      throw new NotFoundException(
        `Quiz '${quizId}' im Kurs '${courseDir}' nicht gefunden`,
      );
    }

    const quiz: QuizDto = JSON.parse(readFileSync(quizPath, 'utf8'));
    return quiz.questions.map(
      ({ correctAnswer, ...rest }) => rest as QuestionWithoutCorrectAnswerDto,
    );
  }
  ...
  addQuizToCourse(
    ...
    if (!existsSync(join(this.baseDir, courseDir))) {
      throw new NotFoundException(
        `Kursverzeichnis '${courseDir}' nicht gefunden.`,
      );
    }
    ...
    writeFileSync(
      join(this.baseDir, courseDir, 'quizzes', quizFileName),
  ...
  addSolutionToQuiz(
    ...
    if (!existsSync(quizPath)) {
      throw new NotFoundException(
        `Quiz '${quizId}' im Kurs '${courseName}' nicht gefunden`,
      );
    }
  ...
  getQuizzesByCourseDir(courseDir: string): { [s: string]: QuizDto } {
    return readdirSync(join(this.baseDir, courseDir, 'quizzes')).reduce(
      (previousValue, currentValue) => {
        const quiz: QuizDto = JSON.parse(
          readFileSync(
            join(this.baseDir, courseDir, 'quizzes', currentValue),
            'utf8',
          ),
        );
